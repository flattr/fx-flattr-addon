...
(function () {
  ...
  var urlbarButton = require('urlbarbutton').UrlbarButton,
    ...
    flattrPattern = new (require("sdk/util/match-pattern").MatchPattern)("*.flattr.com"),
    buttonImage = require("sdk/self").data.url("flattr-button-v2.png"),
    button,
    listeners,
    fillRectRounded,
    buttonSetImageCount,
  ...
  getPageStatus = function (href) {
    if (pb.isPrivate(tabs.activeTab)) {
      return false;
    }
  ...
  checkAPI = function (href, url, callback) {
    if (pb.isPrivate(tabs.activeTab) || href.indexOf('http') !== 0 || (url && url.indexOf('http') !== 0)) {
      if (callback) {
        callback(false);
      }
      return;
